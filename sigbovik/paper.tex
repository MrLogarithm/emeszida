...
Programs in \emeszida\ follow a tabular structure with three main sections: (i) a header, denoted by \texttt{dub} \textcuneiform{𒁾} (``tablet''), (ii) a sequence of instructions, and (iii) a colophon detailing the tablet's authorship.
...
These columns are usually tab-separated, though in a few documents they are TAB-separated (using the cuneiform sign \texttt{TAB} \textcuneiform{𒋰}).
...
\textcuneiform{𒋤} SUD (``distant, remote'') can be used as a null destination, for statements which produce no output.

\paragraph{Opcodes}
Each instruction has a single opcode belonging to the following vocabulary:
\begin{itemize}
    \item \textcuneiform{𒈭𒄩} DAḪ.ḪA, ``add''
    \item \textcuneiform{𒁀𒍣} BA.ZI, ``tear out''
    \item \textcuneiform{𒀀𒁺} A.RA$_2$, ``multiply''
    \item \textcuneiform{𒅆} IGI, ``reciprocal''
    \item \textcuneiform{𒈨} ME, ``to be''
    \item \textcuneiform{𒅆𒂃} \textit{ta-mar}, ``you will see''
    \item \textcuneiform{𒇔𒈾} NIGIN.NA, ``start again''
    \item \textcuneiform{𒋗𒌉𒁉𒋛𒀀} \textit{tukum.bi dirig}, ``if it exceeds''
    \item \textcuneiform{𒋗𒌉𒁉𒋝}  \textit{tukum.bi sig}, ``it it is weak''
\end{itemize}
